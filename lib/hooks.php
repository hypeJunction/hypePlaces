...
function entity_menu_setup($hook, $type, $return, $params)
{
    ...
    if (elgg_is_admin_logged_in()) {
        ...
        $return[] = ElggMenuItem::factory(array('name' => 'feature', 'data-guid' => $entity->guid, 'data-on' => $on_action, 'data-off' => $off_action, 'data-on-text' => $on_text, 'data-off-text' => $off_text, 'data-place' => true, 'data-counter' => 'features', 'data-count' => 0, 'data-status' => $featured ? 'on' : 'off', 'href' => $featured ? $off_action : $on_action, 'text' => $featured ? $off_text : $on_text));
    }
    return $return;
}
/**
...
function interactions_menu_setup($hook, $type, $return, $params)
{
    ...
    if (elgg_is_logged_in()) {
        ...
        $count = elgg_get_entities(array('relationship' => 'bookmarked', 'relationship_guid' => $entity->guid, 'inverse_relationship' => true, 'count' => true));
        ...
        $return[] = ElggMenuItem::factory(array('name' => 'bookmark', 'data-guid' => $entity->guid, 'data-on' => $on_action, 'data-off' => $off_action, 'data-on-text' => $on_text, 'data-off-text' => $off_text, 'data-place' => true, 'data-status' => $bookmarked ? 'on' : 'off', 'link_class' => 'show-counter', 'data-counter' => 'bookmarks', 'data-count' => $count, 'href' => $bookmarked ? $off_action : $on_action, 'text' => $bookmarked ? $off_text : $on_text));
        if ($entity->checkins) {
            ...
            $return[] = ElggMenuItem::factory(array('name' => 'checkin', 'data-guid' => $entity->guid, 'data-on' => $on_action, 'data-off' => $off_action, 'data-on-text' => $on_text, 'data-off-text' => $off_text, 'data-place' => true, 'data-status' => $checked_in ? 'on' : 'off', 'link_class' => 'show-counter', 'data-counter' => 'checkins', 'data-count' => $count, 'href' => $checked_in ? $off_action : $on_action, 'text' => $checked_in ? $off_text : $on_text));
        }
    }
    return $return;
}
...
{
    $return['places'] = array('title' => elgg_echo('places:search'), 'options' => array('id' => 'objects', 'types' => 'object', 'subtypes' => Place::SUBTYPE), 'getter' => 'elgg_get_entities', 'access' => 'public', 'priority' => 500);
    return $return;
}